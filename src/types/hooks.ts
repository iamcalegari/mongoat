...
/**
 * A hook function invoked with an explicit `ctx` object. Sync or async; the
 * return value convention differs between pre and post hooks:
...
/**
 * Post-hook registration entry. When `fireAndForget` is set (opt-in), the
 * hook dispatch is truly non-awaited (does not delay the caller's return)
 * and any rejection is routed to `onHookError` instead of propagating to
 * the caller.
 */
...
/**
 * Callback invoked when a `fireAndForget` post-hook rejects. `err` is typed
 * `unknown` — third-party hooks can throw anything, not necessarily a
 * `MongoatError`/`Error`. Receives the SAME `ctx` the hook itself received
 * (result/document/filter included) — the consumer of `onHookError` is
 * responsible for not logging sensitive fields without redaction.
 */
export type OnHookError<Ctx> = (err: unknown, ctx: Ctx) => void;

/**
 * Declarative per-method hook configuration accepted by
 * `CreateModelProps.hooks`.
 */
...
/**
 * `ctx` shape per `METHODS` value — a lookup type keyed by the method
 * literal so `pre<M extends METHODS>(method: M, fn: (ctx:
 * HookContextMap<ModelType>[M]) => ...)` infers the right ctx from `M`.
 *
 * `ctx.options`/`ctx.filter`/`ctx.document(s)` are the SAME reference used
 * in the driver call — a pre-hook mutation reaches the driver.
 *
 ...
 * being processed while `ctx.documents` stays available as the full batch
 * (e.g. for cross-document checks). Post hooks run ONCE for the whole batch
